...
int main()  {
    ...
    char serverIP[20] = "137.43.168.245";  // IP address of server
    int serverPort = 32980;     // port used by server
    char request[50000];      // array to hold user input
    char reply[50001];        // array to hold received bytes
    char command = '\0';         // character to hold transfer command
...
//    system("mode 50,500");
    ...
    printf("     For a list of user commands enter 'h'\n");
    printf("*************************************************\n");
    ...
    // function htons() converts 16-bit integer to network format
        ...
        if(command == 'h')  {
            printf("\nCHARACTER\tCOMMAND\n"
                   ...
                   "   'u'\t  Upload file to server (will not upload if file already exists)\n"
                   "   'f'\t  Force upload file to server (will replace existing file)\n"
    ...
    /************* Format request *****************/
     ...
     /************* end request formatting *************/
        ...
        // Upload
            ...
            printf("Indicator: %c\n", indicator);
            if(indicator == 'k')    {
                // Send data in blocks of 50000 (refer to Kevin)
                acc_filesize = filesize;
                while(feof(fp) == 0 && ferror(fp) == 0) {
                    read = (int)fread(data, 1, 50000, fp);
                    filesize -= read;
                    ...
                    if(filesize % 100 == 0) print_progress(acc_filesize, acc_filesize - filesize);
                 }
                printf("\rLOADED \n\n");
                TootOwnHorn();
            }
    ...
    /************* Format request *****************/
     ...
     /************ start downloading section *************/
                do  // loop to receive entire reply
                {
                    ...
                    if(totalbytes % 10 == 0)print_progress(filesize, totalbytes);
                    //printf("\nReceived %d bytes\n", totalbytes);
                }
